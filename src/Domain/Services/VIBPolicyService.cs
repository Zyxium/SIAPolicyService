...
public class VIBPolicyService : IVIBPolicyService
{
    ...
    {
        var tranId = DateTime.Now.ToString("yyyyMMddHHmmss");//_tb_agentRepository.CreatePolicyPersonAsync(request);
        ...
        var validate_data = await _VIBPolicyRepository.ValidatePersonTokenAsync(data_viriyah, generate_token.access_token);
        if (validate_data is null)
            throw new CustomHttpBadRequestException("validate_data", "validate_invalid", "validate token found");
